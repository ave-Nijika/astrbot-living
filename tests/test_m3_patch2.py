...
def test_standby_expiry_restores_sleeping():
    """待机结束 + 仍在自主睡眠中（until 未到）→ sleeping 恢复。"""
    gate = make_gate()

    async def _prep():
        await gate.enter_autonomous_sleep(T0 + timedelta(hours=5), "long", T0)
...
# ---------------------------------------------------------------------------
class StandbyGate:
    """待机闸门替身（M6-补丁1：autonomous 是唯一睡眠行为，替身始终醒着，
    在睡状态由 tick/结算路径的其余方法处理）。"""
